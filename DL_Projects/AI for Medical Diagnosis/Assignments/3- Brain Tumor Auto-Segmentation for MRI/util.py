...
def visualize_data_gif(data_, dir_):
    images = []
    for i in range(data_.shape[0]):
        ...
        images.append(img)
    path = os.path.join(dir_,"gif.gif")
    imageio.mimsave(path, images, duration=0.01)
    return Image(filename=path, format='png')
...
def get_labeled_image(image, label, is_categorical=False):
    ...
    if not is_categorical:
        ...
        print("Entered in the else statement")
        labeled_image =  image * (label[:, :, :])
